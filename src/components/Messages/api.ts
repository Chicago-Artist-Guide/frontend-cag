...
import { resolveAccountIdentity } from '../../services/accounts/client';
...
export const createMessageThread = async (
  ...
  const theaterIdentity = await resolveAccountIdentity(theaterAccountId);
  const talentIdentity = await resolveAccountIdentity(talentAccountId);
